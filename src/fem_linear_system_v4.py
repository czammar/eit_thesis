...
aux_nb = (nb/2)
radius = 7. # radio del circulo
center = np.array([[0.0,0.0]]) # centro del circulo
number_circles = 4 # no. de circulos internos que tendra la malla
...
#------------------------------------------------------------------------------
#                    Informacion de nodos y elementos de la malla
#------------------------------------------------------------------------------

mesh_points = cloud_points
...
def triangles_of_electrode(index):
    ...
    # los segmentos [N1,N2] \cup... \cup [Nk, N(k+1)] que forman
...
#------------------------------------------------------------------------------
#             Vector de impedancias de contacto del tejido
#------------------------------------------------------------------------------
...
def local_matrix_plus_centroid(number_triangle):
    ...
    W_K = np.array([[ bi**2+ci**2,  bi*bj+ci*cj, bi*bk+ci*ck],
    ...
    def loc2glb(m):
        ...
        if m==0:
            ...
            return k
    return centroid, W_K, loc2glb

W = np.zeros((number_nodes,number_nodes))
...
for number_triangle in range(len(mesh_elements)):
    centroid , W_K, loc2glb = local_matrix_plus_centroid(number_triangle)
    over_sigma = sigma(centroid)
    W_K = over_sigma* W_K
    for m in range(3):
        for n in range(3):
            W[loc2glb(m)][loc2glb(n)] = W[loc2glb(m)][loc2glb(n)] + W_K[m][n]
...
for i in range(number_nodes):
    for j in range(i,number_nodes):
        for e_index in range(num_electrodes):
             M[i][j] = M[i][j]+ (1./contact_impedance[0][e_index]) *Dot_over_electrode(e_index, i, j)
        M[j][i] = M[i][j]
...
for i in range(number_nodes_boundary):
...
# factor 1/ ( sum z_i * |e_1|)

factor  = 1./( long_electrodes*sum(contact_impedance[0]) )

for m in range(num_electrodes-1):
    for n in range(m,num_electrodes-1):
        z1 = contact_impedance[0][m+1]* contact_impedance[0][n+1]
        z1= factor*z1
        inverse_D[m][n] = z1
        inverse_D[n][m] = inverse_D[m][n] 
        if n == m:
            inverse_D[n][n] = (1./long_electrodes)*contact_impedance[0][n+1] -z1

for i,j in zip(range(15), range(15)):
    print inverse_D[i][j] == inverse_D[j][i]

# C^T D^{-1} C

CDC = np.dot( C ,np.dot(inverse_D, C.T ))
...
system_matrices = [B, C, inverse_D, CDC, B - CDC]

#system_matrices = [  inverse_D ]
...
#print np.linalg.eigvalsh(CDC+Z)
